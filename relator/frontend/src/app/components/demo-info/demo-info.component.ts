...
export class DemoInfoComponent {
  ...
  async doSpeechPrefill() {
    this.loading = true;
    try {
      const transcript = await this.speechAi.getSpeechInput();
      const aiData = await this.speechAi.getAiPrefill(transcript, this.demoForm.value).toPromise();
      this.demoForm.patchValue(aiData);
    } catch (err) {
      alert('Speech/AI error: ' + err);
    }
    this.loading = false;
  }
}